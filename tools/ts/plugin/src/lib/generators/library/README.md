...
- Ensures `tsconfig.lib.json` includes the selected test runner types in `compilerOptions.types` for `testing` libraries.
...
- `unitTestRunner` (`jest` | `vitest` | `none`, default inferred from `type`): Unit test runner.
